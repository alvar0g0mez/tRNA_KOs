...
## 1.1. Load data
```{r}
# Proteomics data - with the PCA outliers removed, without batch correction
...
# Proteomics data - all outliers still included (filter the wrong batch myself later), without batch correction
...
```{r}
# For all KOs together - pretty meaningless I think
ggplot(data = da_full, aes(x = logFC, y = -log10(adj.P.Val), col = Strain.Name)) +
  geom_point() +
...
## Print to PDF
ggsave(
   filename = "C:/MyStuff/tRNAs/Output/Plots/Volcano_plots/volcano_plots_per_KO_all_KOs.pdf", 
...
## Print to PDF
ggsave(
   filename = "C:/MyStuff/tRNAs/Output/Plots/Volcano_plots/volcano_plots_per_KO_KOs_with_DE_proteins.pdf", 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
)
...
phenotypic_data <- as.data.frame(fread('C:/MyStuff/tRNAs/Data/bloom_ackermann_2014/phenotypic_results.tsv'))
...
## Print to PDF
ggsave(
   filename = "C:/MyStuff/tRNAs/Output/Plots/Volcano_plots/volcano_plots_per_KO_UCU_family.pdf", 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
)
...
microarray_data <- as.data.frame(fread("C:/MyStuff/tRNAs/Data/bloom_ackermann_2014/microarray_fold_change_data.tsv"))
...
```{r}
cozen <- read.xlsx("C:/MyStuff/tRNAs/Data/tRNA_expression_comparative/cozen_et_al_2015.xlsx", 1)
