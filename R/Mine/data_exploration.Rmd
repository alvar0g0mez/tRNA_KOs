...
if (working_from == "home") {
  ...
  # The processed proteomics data
  proteomics_processed <- as.data.frame(as.matrix(fread("/home/alvaro/MyStuff/tRNA_KOs/Data/Other/proteomics_data/processed_proteomics_dataframe_batchcorrected_onWTs.tsv"), rownames = 1))
...
Total.Protein.Sum <- c()
Protein.Count <- c()
final_proteomics_colnames <- c()
for (i in 1:ncol(proteomics_processed)) {
  Total.Protein.Sum <- c(Total.Protein.Sum, sum(proteomics_processed[,i]))
  Protein.Count <- c(Protein.Count, sum(!is.na(proteomics_processed[,i])))
  final_proteomics_colnames <- c(final_proteomics_colnames, colnames(proteomics_processed)[i])
}

temp <- data.frame(Total.Protein.Sum, Protein.Count, final_proteomics_colnames)

stats_file_processed <- left_join(temp, stats_file_processed, by = "final_proteomics_colnames")
```

Plots - these are completely useless for now :D
```{r}
