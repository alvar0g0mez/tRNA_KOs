...
```{r}
write_json(go_results_list, path = paste(base_dir, "Data/Other/enrichment_analysis/go_results_list_", alpha_plain, "_logFC_", as.character(lfc_threshold),  ".json", sep=""), pretty = T)
```
...
da <- read_json(paste(base_dir, "Data/Other/enrichment_analysis/da_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".json", sep=""), simplifyVector = T)
da <- da[names(da) != "WT"]
...
# Save these results
write_json(kegg_gsea_results_filtered, paste(base_dir, "Data/Other/enrichment_analysis/kegg_gsea_results_", alpha_plain, "_logFC_", as.character(lfc_threshold), "_enrichment_alpha_", alpha_enrichment_plain, ".json", sep=""), pretty = T)
```
...
# Save these results
write_json(go_gsea_results_filtered, paste(base_dir, "Data/Other/enrichment_analysis/go_gsea_results_", alpha_plain, "_logFC_", as.character(lfc_threshold), "_enrichment_alpha_", alpha_enrichment_plain, ".json", sep=""), pretty = T)
```
...
ggplot(data = phenotypic_data, aes(x = GR_DTT_2014)) +
  geom_histogram(binwidth = 1) +
  theme_light() +
  geom_vline(xintercept = mean(phenotypic_data$GR_DTT_2014, na.rm = T)+sd(phenotypic_data$GR_DTT_2014, na.rm = T), col = "red") +
  geom_vline(xintercept = mean(phenotypic_data$GR_DTT_2014, na.rm = T)-sd(phenotypic_data$GR_DTT_2014, na.rm = T), col = "red")

ggplot(data = phenotypic_data, aes(x = GY_DTT_2014)) +
  geom_histogram(binwidth = 1) +
  theme_light() +
  geom_vline(xintercept = mean(phenotypic_data$GY_DTT_2014, na.rm = T)+sd(phenotypic_data$GY_DTT_2014, na.rm = T), col = "red") +
  geom_vline(xintercept = mean(phenotypic_data$GY_DTT_2014, na.rm = T)-sd(phenotypic_data$GY_DTT_2014, na.rm = T), col = "red")
...
## Actually perform the selection
strains_better_in_DTT <- phenotypic_data$Strain.Name[phenotypic_data$GR_DTT_2014 > sd(phenotypic_data$GR_DTT_2014, na.rm = T) & phenotypic_data$GY_DTT_2014 > sd(phenotypic_data$GY_DTT_2014, na.rm = T)]
strains_worse_or_equal_in_DTT <- phenotypic_data$Strain.Name[phenotypic_data$GR_DTT_2014 < -sd(phenotypic_data$GR_DTT_2014, na.rm = T) & phenotypic_data$GY_DTT_2014 < -sd(phenotypic_data$GY_DTT_2014, na.rm = T)]
```
