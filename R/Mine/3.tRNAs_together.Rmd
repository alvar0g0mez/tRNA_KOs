...
de_proteins_list_tRNAs <- fromJSON(paste(base_dir, "tRNA_KOs/Data/enrichment_analysis/de_proteins_list_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".json", sep=""), )
...
logFC_for_5k <- 0.5
de_proteins_list_5k <- fromJSON(paste(base_dir, "tRNA_KOs/Data/5k/de_proteins_list_", alpha_plain, "_logFC_", as.character(logFC_for_5k), ".json", sep=""), )
...
for (i in 1:length(codons)) {
  ...
  for (j in 1:length(proteins_of_interest)) {
    ...
    list_5k[[protein_of_interest]] <- de_proteins_list_5k[[protein_of_interest]]
    all_5k_de_proteins <- c(all_5k_de_proteins, de_proteins_list_5k[[protein_of_interest]])
  }
  list_5k[["All_5k"]] <- all_5k_de_proteins
  
  ## Add the 5k list to the overall list for this target codon
  list_for_this_codon[["list_5k"]] <- list_5k
  
  
  
  # For the tRNA KOs data
  list_trna_kos <- list()
  all_trna_ko_de_proteins <- c()
  ...
  for (j in 1:length(trna_kos)) {
    trna <- trna_kos[j]
    list_trna_kos[[trna]] <- de_proteins_list_tRNAs[[trna]]
    all_trna_ko_de_proteins <- c(all_trna_ko_de_proteins, de_proteins_list_tRNAs[[trna]])
  }
  list_trna_kos[["All_tRNA_KOs"]] <- all_trna_ko_de_proteins
  
  ## Add the tRNA KO list to the overall list for this target codon
  list_for_this_codon[["list_tRNA_KO"]] <- list_trna_kos
  
  
  
  # Now that the list for this target codon is finished, we append it to the final list
  de_proteins_per_target_codon[[codon]] <- list_for_this_codon
}
```
