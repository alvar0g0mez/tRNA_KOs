...
## 2.3. Add information about anticodon frequency as well
Load and prepare data
```{r}
...
Check from YeastMine how many synthetases there are and come up with a final version of a dataframe with all synthetase information
```{r}
...
# Remove unnecessary variables
rm(detected_proteins, synths_detected, temp, check, synthetases)
```
...
NO IDEA WHAT I MEANT HERE BY DA_FULL, IT'S THE ONLY TIME IT APPEARS IN THE DOCUMENT, NEED TO FIGURE THIS OUT
```{r}
...
## Print to PDF
output_file <- paste(base_dir, "Output/Plots/compare_DEPs_across_KOs/Jaccard_index_heatmaps_per_anticodon.pdf", sep = "")
ggsave(
   filename = output_file, 
...
# Remove unnecessary variables
rm(anticodons, jaccard_matrices_list, anticodon, KOs_with_anticodon, jaccard_matrix, jaccard_matrices_list, plot_list, my_grobs, output_file)
```

Heatmaps by amino acid
```{r}
# Preparations: remove KOs with 0 DE proteins from list, grab anticodon names, create empty list to put Jaccard index matrices there
de_protein_list <- Filter(function(x) length(x) > 0, de_proteins_list_001)
...
## Print to PDF
output_file <- paste(base_dir, "Output/Plots/compare_DEPs_across_KOs/Jaccard_index_heatmaps_per_amino_acid.pdf", sep = "")
ggsave(
   filename = output_file, 
