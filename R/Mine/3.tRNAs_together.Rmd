...
output_file <- paste(base_dir, "Data/Other/enrichment_analysis/de_proteins_list_001_batchcorrected_onWTs.json", sep="")
...
output_file <- paste(base_dir, "Data/Other/enrichment_analysis/de_proteins_list_005_batchcorrected_onWTs.json", sep="")
write_json(de_proteins_list_005, path=output_file)
```

Save all proteins for GSEA
```{r}
write_json(da, path=paste(base_dir, "Data/Other/enrichment_analysis/da_batchcorrected_onWTs.json", sep=""), pretty = T)
```
...
fwrite(responsiveness, paste(base_dir, "Data/responsiveness_batchcorrected_onWTs.csv", sep=""))
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/Volcano_plots/DE_together/volcano_plots_per_KO_all_KOs_together_batchcorrected_onWTs.pdf", sep = "")
ggsave(
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/Volcano_plots/DE_together/volcano_plots_per_KO_KOs_with_DE_proteins_together_batchcorrected_onWTs.pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
...
## Print to PDF
output_location <- paste(base_dir, "Output/Plots/Volcano_plots/DE_together/volcano_plots_per_KO_UCU_family_together_batchcorrected_onWTs.pdf", sep="")
ggsave(
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_isoacceptors_color_by_codon_batchcorrected_onWTs.pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_anticodon_color_by_DNA_seq_batchcorrected_onWTs.pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs_DNA, nrow=4, ncol=4), 
   width = 15, height = 9
)
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_anticodon_color_by_mature_seq_batchcorrected_onWTs.pdf", sep = "")
ggsave(
   filename = output_file, 
...
# NEXT ONE IS P5 IN ALEXIS' ORIGINAL CODE!
```
...
responsiveness <- as.data.frame(fread(paste(base_dir, "Data/responsiveness_batchcorrected_onWTs.csv", sep="")))
```
...
## 3.2. Model the data with a GLM instead
...
Negative binomial - with 3 variables - remove strains below certain numbers of nDEPs
```{r}
# nDEP > 0
