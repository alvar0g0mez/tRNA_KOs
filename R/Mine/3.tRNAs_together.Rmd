...
# Save DE proteins for enrichment analysis
de_proteins_list_001 <- list()
de_proteins_list_001[["background"]] <- rownames(da[[1]])
for (i in 1:length(da)) {
  temp <- da[[i]]
  de_proteins_list_001[[names(da)[i]]] <- rownames(temp)[na.omit(temp$adj.P.Val) < alpha]
}
output_file <- paste(base_dir, "Data/de_proteins_list_001.RData", sep="")
...
# 3. Compare to expression data from Cozen et al., 2015 - THESE ARE JUST READ COUNTS I'M LOOKING AT, HONESTLY NO IDEA IF THIS MAKES SENSE
...
# 4. Repeat 3, but only using genomic tRNA sequencing data from Cozen et al., 2015
...
## 5.1. Try to fit a linear model to predict nDEP based on synthetase abundance and genomic tRNA read counts, and their interaction - not very successful
```{r}
