...
This used to be in section 2, but I think it's better here: - NEED TO MAKE SURE THIS WORKS! HAVEN'T RUN IT YET SINCE I MOVED IT HERE
```{r}
...
Plots - histograms - color by family, amino acid, chromosome
```{r}
...
Manually check the effect of Nt_at_21, looking at what happens exactly for each Nt that can be there
```{r}
D
```
...
## 6.4. Explore intersections between KOs with large nDEPs inside each family
Prepare data
```{r}
...
Analyze
```{r}
# This is what happens if I filter based on major - minor (defined by me) - I end up with only 3 pairs 
DE_protein_overlaps_major_within_family <- DE_protein_overlaps %>%
  filter(anticodon_1 == anticodon_2,
         Gene_type_1 == "Major" & Gene_type_2 == "Major")

# So I'd rather just keep every pair from the same family
```
