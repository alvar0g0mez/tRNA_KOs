...
# Fix names to be that of the strain and not that of the contrast
new_names <- sapply(names(da), function(x) substr(x, 1, str_locate(x, "-")-1))
names(da) <- new_names
```
...
## 1.12. Replicate Alexis' original plots - kind of my own version
```{r}
...
# Color by up-regulated/down_regulated ratio
ggplot(data = od_and_nDEP, aes(x = OD_processed, fill = up_down_regulated_ratio_adjusted)) +
  geom_histogram(binwidth = 0.03, col = "black") +
  theme_light() +
  labs(title = "Processed ODs for tRNA KO strains",
       subtitle = "For the following 96-well plates: 2.1., 2.2., 2.3.",
       fill = "Up/down reg. proteins") +
  ylab("Frequency") +
  xlab("OD")
```
...
## 1.14. Explore phenotypic results (GR and GY)
...
## 1.15. Up-regulated and down-regulated proteins exploration
```{r}
...
add_2_columns <- function(de_synthetases_df) {
  ...
  de_synthetases_df <- left_join(de_synthetases_df, temp, by = "Strain.Name") %>%
    rename(Amino_acid_1_letter = Isotype_from_anticodon_1_letter)
  ...
  de_synthetases_df <- left_join(de_synthetases_df, temp, by = "Amino_acid_1_letter") %>%
    rename(AA_carried_by_tRNA = Amino_acid_1_letter)
  ...
  temp <- amino_acids %>%
    ...
    rename(AA_loaded_by_synthetase = Amino_acid_1_letter,
