...
Dynamically define DE proteins in each strain, taking into account the SD of the logFC instead of a fixed threshold
```{r}
# Iterate over the dfs in the list "da", for each plot the distribution of the logFC and add some vertical lines, probably at mean+2*SD or something like that and see what that looks like, to start figuring out at how many SD we want to set the threshold - also maybe vertical line at logFC = 0.5 and -0.5, to compare to that? Since that is what I am using right now
```
...
ggplot(data = plot_cv_per_strain, aes(x = Strain.Name, y = CV_strain)) +
  ...
  geom_text_repel(data = subset(plot_cv_per_strain, CV_strain > 0.9 | CV_strain < 0.7),
                  aes(x = Strain.Name, y = CV_strain, label = Strain.Name),
                  size = 4)
strains_with_high_CV <- plot_cv_per_strain$Strain.Name[plot_cv_per_strain$CV_strain > 0.9]
```
...
Lollipop plot
```{r}
# Make sure families appear in the order I want them to in the plot
#responsiveness <- responsiveness %>%                    
#  dplyr::arrange(desc(Family_type), anticodon, nDEP)
#responsiveness$anticodon <- factor(responsiveness$anticodon, levels = unique(responsiveness$anticodon))
...
# Plot
ggplot(responsiveness, aes(x = nDEP, y = anticodon, group = anticodon)) +
  geom_line(aes(group = anticodon), color = "grey70") +  # Connects dots within family
  geom_point(aes(color = nDEP), size = 2) +
  scale_color_gradient(low = "lightblue", high = "darkred") +
  labs(
    title = "Differential Expression per tRNA KO (by Family)",
...
TRYING OTHER VERSIONS OF THE PLOT FROM CHATGPT
```{r}
tile_x <- -50
...
my_clusters <- perform_clustering(clust_df, "euclidean", "complete", 5)
```
...
Heatmaps
- Heatmap on the original raw proteomics data
```{r}
...
# Order samples based on families
## Order samples
...
# Create heatmap - letting the algorithm cluster the samples
pdf(paste(base_dir, "tRNA_KOs/Output/Plots/Heatmaps/heatmap_z_score_clustered_samples_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = ""))
ht <- Heatmap(mat_zscore,
        name = "Heatmap",
        #col = col_fun,
        na_col = "green",
...
# Create the Heatmap - order samples based on amino acid carried (per strain also)
...
# Get the tags for some of the PCA plot versions
PCA_tags_outlying_strains <- get_PCA_tags(PCA_data, sample_layout, "outlying_strains")
```
...
Create fake data that follows our hypothesis - for comparison to the real results, for the poster
```{r}
# Create fake data
fake_data <- list()
for (i in 1:length(codon_ratios_list)) {
...
logFC_for_5k <- 0.5
de_proteins_list_5k <- fromJSON(paste(base_dir, "tRNA_KOs/Data/5k/de_proteins_list_up_down_", alpha_plain, "_logFC_", as.character(logFC_for_5k), ".json", sep=""), )
...
for (i in 1:length(codons)) {
  ...
  for (j in 1:length(proteins_of_interest)) {
    ...
    list_5k_up[[protein_of_interest]] <- de_proteins_list_5k[[protein_of_interest]][["Up"]]
    all_5k_de_proteins_up <- c(all_5k_de_proteins_up, de_proteins_list_5k[[protein_of_interest]][["Up"]])
    
    list_5k_down[[protein_of_interest]] <- de_proteins_list_5k[[protein_of_interest]][["Down"]]
    all_5k_de_proteins_down <- c(all_5k_de_proteins_down, de_proteins_list_5k[[protein_of_interest]][["Down"]])
    
    list_5k_all[[protein_of_interest]] <- c(de_proteins_list_5k[[protein_of_interest]][["Down"]], de_proteins_list_5k[[protein_of_interest]][["Up"]])
    all_5k_de_proteins_all <- c(all_5k_de_proteins_all, list_5k_all[[protein_of_interest]])
  }
  list_5k_up[["All_5k"]] <- all_5k_de_proteins_up
  list_5k_down[["All_5k"]] <- all_5k_de_proteins_down
  list_5k_all[["All_5k"]] <- all_5k_de_proteins_all
  
  ## Add the 5k list to the overall list for this target codon
  list_for_this_codon_up[["list_5k"]] <- list_5k_up
  list_for_this_codon_down[["list_5k"]] <- list_5k_down
  list_for_this_codon_all[["list_5k"]] <- list_5k_all
  ...
  for (j in 1:length(trna_kos)) {
    trna <- trna_kos[j]
    list_trna_kos_up[[trna]] <- de_proteins_list_tRNAs[[trna]][["Up"]]
    all_trna_ko_de_proteins_up <- c(all_trna_ko_de_proteins_up, de_proteins_list_tRNAs[[trna]][["Up"]])
    
    list_trna_kos_down[[trna]] <- de_proteins_list_tRNAs[[trna]][["Down"]]
    all_trna_ko_de_proteins_down <- c(all_trna_ko_de_proteins_down, de_proteins_list_tRNAs[[trna]][["Down"]])
    
    list_trna_kos_all[[trna]] <- c(de_proteins_list_tRNAs[[trna]][["Up"]], de_proteins_list_tRNAs[[trna]][["Down"]])
    all_trna_ko_de_proteins_all <- c(all_trna_ko_de_proteins_all, list_trna_kos_all[[trna]])
  }
  list_trna_kos_up[["All_tRNA_KOs"]] <- all_trna_ko_de_proteins_up
  list_trna_kos_down[["All_tRNA_KOs"]] <- all_trna_ko_de_proteins_down
  list_trna_kos_all[["All_tRNA_KOs"]] <- all_trna_ko_de_proteins_all
  
  ## Add the tRNA KO list to the overall list for this target codon
  list_for_this_codon_up[["list_tRNA_KO"]] <- list_trna_kos_up
  list_for_this_codon_down[["list_tRNA_KO"]] <- list_trna_kos_down
  list_for_this_codon_all[["list_tRNA_KO"]] <- list_trna_kos_all
...
my_grobs <- lapply(plot_list, ggplotGrob)
output_file <- paste(base_dir, "tRNA_KOs/Output/Plots/compare_codon_content/check_in_5_prime_end/Jaccard_index_heatmaps_5k_vs_tRNA_KOs_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
