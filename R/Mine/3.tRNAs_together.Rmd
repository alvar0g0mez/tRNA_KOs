...
### 1.4.2. Perform DE analysis - with limpa! Using this one from now on
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/Volcano_plots/volcano_plots_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_isoacceptors_color_by_codon_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_anticodon_color_by_DNA_seq_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs_DNA, nrow=4, ncol=4), 
   width = 15, height = 9
)
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_anticodon_color_by_mature_seq_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs_mature, nrow=4, ncol=4), 
   width = 15, height = 9
)
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_anticodon_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_anticodon_only_1_major_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_anticodon_more_than_1_major_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_anticodon_nDEP_below_20_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
...
resp_and_expression <- as.data.frame(fread(paste(base_dir, "Data/resp_and_expression_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".csv", sep="")))
...
## 1.10. Number of DE proteins per family, coloring by number of tRNAs in the family
```{r}
...
## 1.11. Do all tRNAs with the same anticodon have exactly the same sequence throughout their whole structure?
...
## 1.12. Explore correlation between the number of tRNA genes for a certain amino acid (or anticodon) that couldn't be KOd, and the number of genes with that amino acid/anticodon - THIS WAS IN A DIFFERENT FILE, IN THE TSSC CODE! NOT SURE IF IT WILL RUN HERE BUT I THINK IT SHOULD
```{r}
# Plots by anticodon
...
## 1.13. Replicate Alexis' original plots - kind of my own version
```{r}
...
## 1.15. Explore phenotypic results (GR and GY)
...
## 1.16. Up-regulated and down-regulated proteins exploration
```{r}
...
responsiveness <- as.data.frame(fread(paste(base_dir, "Data/responsiveness_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".csv", sep="")))
```
...
```{r}
fwrite(resp_and_expression, paste(base_dir, "Data/resp_and_expression_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".csv", sep=""))
...
```{r}
# Load list
codon_ratios_list <- fromJSON(txt = paste(base_dir, "Data/Other/check_codon_enrichment_in_protein_sequences/codon_counts.json", sep=""))
...
## Print to PDF
my_grobs <- lapply(plot_list, ggplotGrob)
output_file <- paste(base_dir, "Output/Plots/compare_DEPs_across_KOs/jaccard/Jaccard_index_heatmaps_per_anticodon_no_NAs_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
...
## Print to PDF
output_file <- paste(base_dir, "Output/Plots/compare_DEPs_across_KOs/jaccard/Jaccard_index_heatmaps_per_amino_acid_no_NAs_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
...
## Print to PDF
output_file <- paste(base_dir, "Output/Plots/compare_DEPs_across_KOs/jaccard/Jaccard_index_heatmaps_per_anticodon_with_NAs_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=3, ncol=3), 
   width = 15, height = 9
...
## Print to PDF
output_file <- paste(base_dir, "Output/Plots/compare_DEPs_across_KOs/jaccard/Jaccard_index_heatmaps_per_amino_acid_with_NAs_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=2, ncol=3), 
   width = 15, height = 9
...
```{r}
fwrite(DE_protein_overlaps, paste(base_dir, "Data/Other/compare_DEPs_across_KOs/DE_protein_overlaps_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".tsv", sep=""))
```
...
DE_protein_overlaps <- as.data.frame(fread(paste(base_dir, "Data/Other/compare_DEPs_across_KOs/DE_protein_overlaps_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".tsv", sep="")))
...
Look at the EA results for these strains - this is the EA done in the main file, but since I don't find anything to be enriched in most of the strains, I am going to repeat it here only for these strains trying to be more lenient with the p-values and other thresholds. I don't know if this is a great idea, these I wouldn't interpret generally as pathways or whatever that are enriched in these strains, but rather I am using it simply to summarize the proteins that are DE in these strains in a more readable way so that I can compare them within the pair, I cannot look at 2 lists of 40 proteins each and compare that myself. 
```{r}
# Load GO EA results
go_results_list <- read_json(path = paste(base_dir, "Data/Other/enrichment_analysis/go_results_list_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".json", sep=""), simplifyVector = T)
...
go_results_list_filtered <- go_results_list[names(go_results_list_001) %in% paired_major_strains]
...
go_gsea_results <- read_json(path = paste(base_dir, "Data/Other/enrichment_analysis/go_gsea_results_", alpha_plain, "_logFC_", as.character(lfc_threshold), "_enrichment_alpha_", alpha_enrichment_plain, ".json", sep=""), simplifyVector = T)
...
# Load the GO GSEA results produced in that file
kegg_gsea_results <- read_json(path = paste(base_dir, "Data/Other/enrichment_analysis/kegg_gsea_results_", alpha_plain, "_logFC_", as.character(lfc_threshold), "_enrichment_alpha_", alpha_enrichment_plain, ".json", sep=""), simplifyVector = T)
...
FIGURE OUT WHAT IS GOING ON WITH THE NEW VERSION OF THE DE ANALYSIS - DELETE ONCE THAT'S DONE
```{r}
da_old <- read_json(paste(base_dir, "Data/Other/enrichment_analysis/da_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".json", sep=""), simplifyVector = T)
da_new <- read_json(paste(base_dir, "Data/Other/enrichment_analysis/da_NEW_TRIAL_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".json", sep=""), simplifyVector = T)

responsiveness_old <- as.data.frame(fread(paste(base_dir, "Data/responsiveness_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".csv", sep="")))
responsiveness_new <- as.data.frame(fread(paste(base_dir, "Data/responsiveness_NEW_TRIAL_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".csv", sep="")))
```
