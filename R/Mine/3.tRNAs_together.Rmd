...
I AM NOT SURE HOW TO DEFINE THE COLUMN NAMES - THEY NEED TO BE DIFFERENT, BUT THEN WHEN I DO THE DESIGN MATRIX THEY NEED TO BE THE SAME? I DON'T REMEMBER HOW ALEXIS DID IT
```{r}
...
## Print to PDF
ggsave(
   filename = "C:/MyStuff/tRNAs/Output/volcano_plots_per_KO_all_KOs.pdf", 
...
for (i in 1:length(KOs_with_DE_proteins)) {
  ...
  temp <- da_only_KOs_with_DE_proteins %>% filter(ko == trna)
  plot_list[[i]] <- ggplot(data = temp, aes(x = logFC, y = -log10(adj.P.Val), col = diffexpressed)) +
    geom_point() +
    geom_hline(yintercept = -log10(0.01), linetype = "dashed", col = "gray") +
    geom_vline(xintercept = c(-1.5, 1.5), linetype = "dashed", col = "gray") +
...
## Print to PDF
ggsave(
   filename = "C:/MyStuff/tRNAs/Output/volcano_plots_per_KO_KOs_with_DE_proteins.pdf", 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
)
...
UCU_family <- full_join(UCU_major_minor, UCU_responsiveness, by = "ko")


# Volcano plots
## Keep only KOs in UCU family
KOs_in_UCU_family <- UCU_family$ko
da_only_KOs_from_UCU_family <- da[da$ko %in% KOs_in_UCU_family,]

## Come up with the list of plots
plot_list <- list()
for (i in 1:length(KOs_in_UCU_family)) {
  trna <- KOs_in_UCU_family[i]
  plot_title <- paste(trna, UCU_family$UCU_family[UCU_family$ko == trna], sep = "_")
  temp <- da_only_KOs_from_UCU_family %>% filter(ko == trna)
  plot_list[[i]] <- ggplot(data = temp, aes(x = logFC, y = -log10(adj.P.Val), col = diffexpressed)) +
    geom_point() +
    geom_hline(yintercept = -log10(0.01), linetype = "dashed", col = "gray") +
    geom_vline(xintercept = c(-1.5, 1.5), linetype = "dashed", col = "gray") +
...
## Print to PDF
ggsave(
   filename = "C:/MyStuff/tRNAs/Output/volcano_plots_per_KO_UCU_family.pdf", 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
)
...
--> COULD I FIND A BIMODAL DISTRIBUTION OF THE nDEPs IN EACH TRNA FAMILY, ONE FOR MAJOR AND ONE FOR MINOR TRNA GENES? maybe not enough trnas per family to find such thing as a distribution but yeah
...
## 1.10. Enrichment analysis - by Alexis
```{r}
ea <- list()
...
```{r}
# Histogram of nDEP for each KO strain
ggplot(data = responsiveness, aes(x = nDEP)) +
  geom_histogram(binwidth = 1, color="black", fill="grey") +
  theme_light()

# There are ~80 KO strains with 0 nDEP, I don't think it's that straightforward but for now I'll assume those are the minor tRNA genes

# Define a column with "minor"-"major" based on this so that I can check how many major and minor ones there are per family (I could also do this directly based on the nDEP tbh but oh well)
responsiveness <- responsiveness %>%
...
```{r}
# Load their phenotypic data: growth rate and growth yield for all KOs
phenotypic_data <- as.data.frame(fread('C:/MyStuff/tRNAs/Data/phenotypic_results.tsv'))

# Change "ko" colname to allow to merge datasets based on it
colnames(responsiveness)[colnames(responsiveness) == "ko"] <- "gene_name"
test <- full_join(phenotypic_data, responsiveness, by = "gene_name")
...
microarray_data <- as.data.frame(fread("C:/MyStuff/tRNAs/Data/microarray_fold_change_data.tsv"))
...
for (i in 1:length(microarray_samples)) {
  ...
  corr_coef <- round(cor(test$logFC, test$logfold_microarray, method = "spearman"), 2)
...
## 3.4. Join master dataset and phenotypic results from Bloom-Ackermann et al., 2014
```{r}
# 2. Bring both datasets together into a final master dataset
...
# 2.1. Since the anticodon sequences are different (in the database they are in DNA,
# e.g. TGC, while in the phenotypic results they are in RNA, e.g. UGC), I will take the opportunity
# to tag this before I merge them. Honestly I don't know why they keep them in DNA in the dabase,
# since in the GtRNADB_name they are in RNA, but oh well
colnames(db)[colnames(db) == "Anticodon"] <- "Anticodon_DNA"
colnames(phenotypic_results)[colnames(phenotypic_results) == "anticodon"] <- "Anticodon_RNA"


# 2.2. Join to their phenotypic results dataset
master_dataset <- left_join(db, phenotypic_results, by = "GtRNADB_name")


# 2.4. Check that all tRNAs from both the database and the phenotypic results are included in the final dataset
length(phenotypic_results$GtRNADB_name[which(!(phenotypic_results$GtRNADB_name %in% master_dataset$GtRNADB_name))]) == 0
length(db$GtRNADB_name[which(!(db$GtRNADB_name %in% master_dataset$GtRNADB_name))]) == 0


## 2.5. Change gene_name column to Strain.Name so that it maps to the sample_layout dataset
colnames(master_dataset)[colnames(master_dataset) == "gene_name"] <- "Strain.Name"


## Remove unnecessary variables
rm(GtRNADB_names, new_name, og_name, part_to_be_replaced, replacement, roman_num, i, phenotypic_results, db)
```
...
TRYING TO FIND A DATASET WITH EXPRESSION OF TRNA GENES IN S CEREVISIAE
```{r}
library(data.table)

# alhaj_sulaiman
df <- as.data.frame(fread("C:/Users/algo12/Downloads/GSE201413.final.pcl"))
alhaj_sulaiman <- df[, grepl("Wild Type, untreated", colnames(df)) | grepl("NAME", colnames(df))]

# Bose 2012
df <- as.data.frame(fread("C:/Users/algo12/Downloads/GSE27235.final.pcl"))

# Chu 2013
df <- as.data.frame(fread("C:/Users/algo12/Downloads/GSE47713.final.pcl"))
```
