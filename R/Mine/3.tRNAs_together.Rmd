...
## Print to PDF
output_file = paste(base_dir, "tRNA_KOs/Output/Plots/nDEP_per_families/histograms/nDEP_per_families/histograms/nDEP_hists_per_isoacceptors_color_by_codon_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
...
responsiveness <- responsiveness %>%
  dplyr::mutate(Family_type = case_when(Major_count == 1 ~ "One major",
                                        Major_count > 1 ~ "More than one major",
                                        Major_count < 1 ~ "All low"))
...
temp_1 <- responsiveness %>%
  dplyr::filter(Family_type == "All low" | Family_type == "More than one major")
...
temp_2 <- responsiveness %>%
  dplyr::filter(Family_type == "One major")
...
annotation_data <- responsiveness %>%
  dplyr::select(anticodon, Family_type) %>%
  dplyr::distinct() %>%
  dplyr::mutate(x = -10)
...
lollipop_plot_final <- ggplot(responsiveness, aes(x = nDEP, y = anticodon)) +
  ...
  geom_tile(data = annotation_data, aes(fill = Family_type), x = x, width = 20, height = 0.8) +
  
  # Lollipop line and point
  geom_line(aes(group = anticodon), color = "grey70") +
  geom_point(aes(color = nDEP), size = 2.5) +
  
  # Scales and labels
  scale_color_gradient(low = "lightblue", high = "darkred") +
  scale_fill_manual(values = c("All low" = "#BFD3E6", 
                               "More than one major" = "#F6BD60", 
                               "One major" = "#F28482")) +
