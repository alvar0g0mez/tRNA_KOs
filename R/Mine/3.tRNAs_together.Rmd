...
fwrite(responsiveness, paste(base_dir, "Data/responsiveness_", alpha_plain, "_logFC_", as.character(logFC_threshold), ".csv", sep=""))
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/Volcano_plots/volcano_plots_per_KO_all_KOs_", alpha_plain, "_logFC_", as.character(logFC_threshold), ".pdf", sep = "")
ggsave(
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/Volcano_plots/volcano_plots_per_KO_KOs_with_DE_proteins_", alpha_plain, "_logFC_", as.character(logFC_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
...
## Print to PDF
output_location <- paste(base_dir, "Output/Plots/Volcano_plots/volcano_plots_per_KO_UCU_family_", alpha_plain, "_logFC_", as.character(logFC_threshold), ".pdf", sep="")
ggsave(
...
responsiveness <- responsiveness %>%
  ...
  mutate(Gene_type = case_when(nDEP > 0.55*family_max ~ "Major",                      # Current version
                               TRUE ~ "Minor")) %>%
  group_by(anticodon) %>%
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_isoacceptors_color_by_codon_", alpha_plain, "_logFC_", as.character(logFC_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_anticodon_color_by_DNA_seq_", alpha_plain, "_logFC_", as.character(logFC_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs_DNA, nrow=4, ncol=4), 
   width = 15, height = 9
)
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_anticodon_color_by_mature_seq", alpha_plain, "_logFC_", as.character(logFC_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs_mature, nrow=4, ncol=4), 
   width = 15, height = 9
)
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_anticodon", alpha_plain, "_logFC_", as.character(logFC_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
...
fwrite(responsiveness, paste(base_dir, "Data/responsiveness", alpha_plain, "_logFC_", as.character(logFC_threshold), ".csv", sep=""))
```
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_anticodon_only_1_major", alpha_plain, "_logFC_", as.character(logFC_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
...
resp_and_expression <- as.data.frame(fread(paste(base_dir, "Data/resp_and_expression", alpha_plain, "_logFC_", as.character(logFC_threshold), ".csv", sep="")))
...
## Remove WT dataframe
de_proteins_list <- de_proteins_list_001[names(de_proteins_list_001) != "WT"]
...
## Print to PDF
my_grobs <- lapply(plot_list, ggplotGrob)
output_file <- paste(base_dir, "Output/Plots/compare_DEPs_across_KOs/jaccard/Jaccard_index_heatmaps_per_anticodon_no_NAs.pdf", sep = "")
ggsave(
   filename = output_file, 
...
## Print to PDF
output_file <- paste(base_dir, "Output/Plots/compare_DEPs_across_KOs/jaccard/Jaccard_index_heatmaps_per_amino_acid_no_NAs.pdf", sep = "")
ggsave(
   filename = output_file, 
...
## Print to PDF
output_file <- paste(base_dir, "Output/Plots/compare_DEPs_across_KOs/jaccard/Jaccard_index_heatmaps_per_anticodon_with_NAs.pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=3, ncol=3), 
   width = 15, height = 9
...
## Print to PDF
output_file <- paste(base_dir, "Output/Plots/compare_DEPs_across_KOs/jaccard/Jaccard_index_heatmaps_per_amino_acid_with_NAs.pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=2, ncol=3), 
   width = 15, height = 9
...
## Define background
temp <- data.frame("Protein_stable_ID" = de_proteins_list_001[["WT"]])
...
for (i in 1:N) {
  ...
  proteins <- intersect(de_proteins_list_001[[strain_1]], de_proteins_list_001[[strain_2]])
...
Quick (and more lenient) EA only for these strains - only for proteins DE at 0.05 for now
- Load data
```{r}
# List of dataframes with the proteins that were DE in each KO strain - at alpha = 0.01 - remove emtpy vectors
de_proteins_list_001 <- fromJSON(paste(base_dir, "Data/Other/enrichment_analysis/de_proteins_list_001.json", sep=""))
...
- Get NCBI gene IDs for each of the systematic name vectors I have
```{r}
...
- Get the information of how many DE proteins there are for each tRNA KO strain
```{r}
