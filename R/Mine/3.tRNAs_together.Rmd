...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/Volcano_plots/volcano_plots_per_KO_all_KOs.pdf", sep = "")
ggsave(
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/Volcano_plots/volcano_plots_per_KO_KOs_with_DE_proteins.pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
...
## Print to PDF
output_location <- paste(base_dir, "Output/Plots/Volcano_plots/volcano_plots_per_KO_UCU_family.pdf", sep="")
ggsave(
...
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_anticodon.pdf", sep = "")
ggsave(
   filename = output_file, 
...
## Print to PDF
output_file = paste(base_dir, "Output/Plots/nDEP_hists_per_anticodon_only_1_major.pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
...
Do this for the DE proteins at 0.01 or 0.05?
```{r}
if (alpha == 0.01) {
  de_protein_list <- de_proteins_list_001
}
if (alpha == 0.05) {
  de_protein_list <- de_proteins_list_005
}
```
...
There are only 42 such pairs out of 19,306 combinations
```{r}
...
codons <- unique(sapply(names(go_gsea_results_filtered), extract_anticodon_from_trna_name))

# Iterate over codons and create a table for each pair of KOs (for those anticodons for which we have more than 1 KO)
for (i in 1:length(codons)) {
  codon <- codons[i]
  temp_go_gsea_results <- go_gsea_results_filtered[grepl(codon, names(go_gsea_results_filtered))]
  
  # Proceed only if there are GSEA results for at least 2 KOs with this anticodon
  if (length(temp_go_gsea_results) > 1) {
    temp_list <- list()
    for (j in 1:length(temp_go_gsea_results)) {
      temp_list[[names(temp_go_gsea_results)[j]]] <- sort(temp_go_gsea_results[[j]]$pathway)
    }
