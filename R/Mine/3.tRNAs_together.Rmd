...
# Sample layout
sample_layout <- as.data.frame(fread(paste(base_dir, "tRNA_KOs/Data/basic/metadata_alvaro.tsv", sep="")))
...
sample_layout_present <- sample_layout[sample_layout$final_proteomics_colnames %in% colnames(y.protein$E),]

# Run the DE analysis - generate da and responsiveness dataframes
de_analysis_results <- de_analysis(raw_data = y.protein,
                                   metadata = sample_layout_present,
                                   method = "limpa",
...
# Save dataframes
fwrite(da, file = paste(base_dir, "tRNA_KOs/Data/enrichment_analysis/da_df_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".csv", sep=""))
...
proteomics_data_non_log2_long <- left_join(proteomics_data_non_log2_long, proteomics_layout, by = "final_proteomics_colnames") %>%
  ...
  dplyr::mutate(CV_strain = robust_cv(value)) %>%                    # CV for each protein across samples of the same family (biological replicates)
  ungroup() %>%
  group_by(protein) %>%
...
ggplot(data = plot_cv_per_protein, aes(x = protein, y = CV_protein)) +
  ...
  geom_text_repel(data = subset(plot_cv_per_protein, CV_protein > 0.6),
                  aes(x = protein, y = CV_protein, label = protein),
                  size = 4)
proteins_with_high_CV <- plot_cv_per_protein$protein[plot_cv_per_protein$CV_protein > 0.6]
...
ggplot(data = plot_cv_per_strain, aes(x = Strain.Name, y = CV_strain)) +
  ...
  geom_text_repel(data = subset(plot_cv_per_strain, CV_strain > 0.25 | CV_strain < 0.1),
...
plot_cv_per_strain <- left_join(plot_cv_per_strain, resp_temp, by = "Strain.Name")

ggplot(data = plot_cv_per_strain, aes(x = CV_strain, y = nDEP)) +
  geom_point() +
  theme_light() +
  xlab("Average CV per strain") +
  ylab("nDEP") +
  labs(title = "Average CV per strain (at protein level) vs. nDEP") +
  geom_text_repel(data = subset(plot_cv_per_strain, nDEP > 500|CV_strain > 0.4), 
                  aes(x = CV_strain, y = nDEP, label = Strain.Name))
```

Look at expression of each of the proteins with the high CV in detail
```{r}
## AM I FIXING THIS ELSEWHERE??? THERE ARE NO REPEATED SAMPLES AT THIS POINT ANYMORE
...
## Print to PDF
output_file = paste(base_dir, "tRNA_KOs/Output/Plots/nDEP_per_families/histograms/nDEP_hists_per_isoacceptors_color_by_codon_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
...
## Print to PDF
output_file = paste(base_dir, "tRNA_KOs/Output/Plots/nDEP_per_families/histograms/nDEP_hists_per_anticodon_color_by_DNA_seq_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs_DNA, nrow=4, ncol=4), 
   width = 15, height = 9
)
output_file = paste(base_dir, "tRNA_KOs/Output/Plots/nDEP_per_families/histograms/nDEP_hists_per_anticodon_color_by_mature_seq_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs_mature, nrow=4, ncol=4), 
   width = 15, height = 9
)
output_file = paste(base_dir, "tRNA_KOs/Output/Plots/nDEP_per_families/histograms/nDEP_hists_per_anticodon_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
...
## Print to PDF
output_file = paste(base_dir, "tRNA_KOs/Output/Plots/nDEP_per_families/histograms/nDEP_hists_per_anticodon_only_1_major_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
...
## Print to PDF
output_file = paste(base_dir, "tRNA_KOs/Output/Plots/nDEP_per_families/histograms/nDEP_hists_per_anticodon_more_than_1_major_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
...
## Print to PDF
output_file = paste(base_dir, "tRNA_KOs/Output/Plots/nDEP_per_families/histograms/nDEP_hists_per_anticodon_nDEP_below_20_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=4, ncol=4), 
   width = 15, height = 9
...
### 1.7.2. Better version of these barplots
Dot matrix heatmap - could possibly delete this one, I don't think it's really useful
```{r}
...
# Divide the data into 2 pieces so we can plot each of them separately for better readability
## First part
temp_1 <- responsiveness %>%
  dplyr::filter(Family_type == "All minor tRNAs" | Family_type == "Multiple major tRNAs")

ggplot(temp_1, aes(x = Strain.Name, y = anticodon, color = nDEP)) +
  geom_point(size = 2) +
  scale_color_gradient(low = "lightblue", high = "firebrick",
                       limits = c(0, max(responsiveness$nDEP))) +
  labs(
    title = "Differential Expression per tRNA KO",
    x = "tRNA Gene",
    y = "tRNA Family",
    color = "# DE Proteins"
  ) +
...
## Second part
temp_2 <- responsiveness %>%
  dplyr::filter(Family_type == "One major tRNA")

ggplot(temp_2, aes(x = Strain.Name, y = anticodon, color = nDEP)) +
  geom_point(size = 2) +
  scale_color_gradient(low = "lightblue", high = "firebrick",
                       limits = c(0, max(responsiveness$nDEP))) +
  labs(
...
Lollipop plot - nicest one I think
```{r}
tile_x <- -50
...
ggsave(filename = "lollipop.svg",
       ...
       path = paste(base_dir, "tRNA_KOs/Output/Plots/nDEP_per_families/", sep = ""),
...
### 1.7.3. Relating family size, members of family in the experiment, and % of major members
...
#amino_acids <- as.data.frame(fread(paste(base_dir, "tRNA_KOs/Data/databases/GtRNAdb/amino_acids.csv", sep=""))) 
#amino_acids_temp <- amino_acids %>%
#  dplyr::select(Amino_acid_3_letter, Amino_acid_1_letter)
#synthetases <- left_join(synthetases, amino_acids_temp, by = "Amino_acid_3_letter")
temp <- synthetases %>%
...
## 1.10. Explore correlation between the number of tRNA genes for a certain amino acid (or anticodon) that couldn't be KOd, and the number of genes with that amino acid/anticodon - THIS WAS IN A DIFFERENT FILE, IN THE TSSC CODE!
```{r}
# Plots by anticodon
...
Compare the phenotypic results from 2014 and 2020 - pretty comparable
```{r}
...
pdf(paste(base_dir, "tRNA_KOs/Output/Plots/dendrograms/dist_method_",  distance_method, "_clust_method_", clustering_method, ".pdf", sep=""), width=40, height=15)
...
rotate_pdf(
  page_rotation = 90,
  input_filepath = paste(base_dir, "tRNA_KOs/Output/Plots/dendrograms/dist_method_",  distance_method, "_clust_method_", clustering_method, ".pdf", sep=""),
  output_filepath = paste(base_dir, "tRNA_KOs/Output/Plots/dendrograms/dist_method_",  distance_method, "_clust_method_", clustering_method, ".pdf", sep=""),
  overwrite = TRUE
)
```
...
create_heatmap(protein_mat = trna_ko,
               ...
               location_to_save = paste(base_dir, "tRNA_KOs/Output/Plots/Heatmaps/", sep=""),
               save_as = "png",
               plot_name = "default",
...
do_pca(trna_ko,
       ...
       metadata_column_for_sample_name = "final_proteomics_colnames",
       location_to_save = paste(base_dir, "tRNA_KOs/Output/Plots/PCA/", sep=""),
       save_as = "png")
```
...
ggsave(filename = "target_codon_proportion_volcano_plot.svg",
       plot = p,
       path = paste(base_dir, "tRNA_KOs/Output/Plots/compare_codon_content/", sep=""),
...
write_json(codons, 
           path = paste(base_dir, "tRNA_KOs/Data/check_codon_enrichment_in_protein_sequences/check_in_5_prime_end/unique_target_codons.json", sep=""))
```
...
initiation_rate_oligo_dt <- as.data.frame(fread(paste(base_dir, "tRNA_KOs/Data/Articles/riba_2019/sup_dataset_1.tsv", sep="")))
...
initation_rate_ribo_zero <- as.data.frame(fread(paste(base_dir, "tRNA_KOs/Data/Articles/riba_2019/sup_dataset_2.tsv", sep="")))
...
messner_5k <- as.data.frame(fread(paste(base_dir, "tRNA_KOs/Data/5k/yeast5k_stat_DE.csv", sep="")))

# UniProt dataset (rows in Messner et al., 2023 dataset are UniProt IDs)
uniprot_db <- as.data.frame(fread(paste(base_dir, "tRNA_KOs/Data/databases/uniprotkb_proteome_UP000002311_2025_05_24.tsv", sep="")))

# DE proteins in tRNA_KOs
de_proteins_list_tRNAs <- fromJSON(paste(base_dir, "tRNA_KOs/Data/enrichment_analysis/de_proteins_list_up_down_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".json", sep=""), )
...
logFC_for_5k <- 0.5
de_proteins_list_5k <- readRDS(paste(base_dir, "tRNA_KOs/Data/5k/de_proteins_list_up_down_", alpha_plain, "_logFC_", as.character(logFC_for_5k), ".RData", sep=""))
...
my_grobs <- lapply(plot_list, ggplotGrob)
output_file <- paste(base_dir, "tRNA_KOs/Output/Plots/compare_codon_content/check_in_5_prime_end/Jaccard_index_heatmaps_5k_vs_tRNA_KOs_all_DE_proteins_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
...
# Save the plots to a PDF
my_grobs <- lapply(plot_list, ggplotGrob)
output_file <- paste(base_dir, "tRNA_KOs/Output/Plots/compare_codon_content/check_in_5_prime_end/Jaccard_index_heatmaps_5k_vs_tRNA_KOs_up_proteins_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=2, ncol=2), 
   width = 15, height = 9
...
# Save the plots to a PDF
my_grobs <- lapply(plot_list, ggplotGrob)
output_file <- paste(base_dir, "tRNA_KOs/Output/Plots/compare_codon_content/check_in_5_prime_end/Jaccard_index_heatmaps_5k_vs_tRNA_KOs_down_proteins_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=2, ncol=2), 
   width = 15, height = 9
...
# Save the plots to a PDF
my_grobs <- lapply(plot_list, ggplotGrob)
output_file <- paste(base_dir, "tRNA_KOs/Output/Plots/compare_codon_content/check_in_5_prime_end/Jaccard_index_heatmaps_high_initiation_rate_5k_vs_tRNA_KOs_all_DE_proteins_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=2, ncol=2), 
   width = 15, height = 9
...
# Save the plots to a PDF
my_grobs <- lapply(plot_list, ggplotGrob)
output_file <- paste(base_dir, "tRNA_KOs/Output/Plots/compare_codon_content/check_in_5_prime_end/Jaccard_index_heatmaps_high_initiation_rate_5k_vs_tRNA_KOs_up_proteins_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=2, ncol=2), 
   width = 15, height = 9
...
# Save the plots to a PDF
my_grobs <- lapply(plot_list, ggplotGrob)
output_file <- paste(base_dir, "tRNA_KOs/Output/Plots/compare_codon_content/check_in_5_prime_end/Jaccard_index_heatmaps_high_initiation_rate_5k_vs_tRNA_KOs_down_proteins_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".pdf", sep = "")
ggsave(
   filename = output_file, 
   plot = marrangeGrob(my_grobs, nrow=2, ncol=2), 
   width = 15, height = 9
...
# da 
da <- fread(paste(base_dir, "tRNA_KOs/Data/enrichment_analysis/da_df_", alpha_plain, "_logFC_", as.character(lfc_threshold), ".csv", sep=""))
...
sample_layout_family_size <- left_join(sample_layout, master_dataset_temp, by = "Strain.Name") %>%
  dplyr::mutate(Family_size_cat = case_when(Strain.Name == "WT" ~ "WT",
                                            TRUE ~ Family_size_cat))

create_heatmap(protein_mat = heatmap_df,
               annotation = "single",
               metadata = sample_layout_family_size,
               metadata_column_for_sample_name = "final_proteomics_colnames",
...
# Open a PDF for plotting; units are inches by default
pdf(paste(base_dir, "tRNA_KOs/Output/Plots/dendrograms/RNA_pol_III_dist_method_",  distance_method, "_clust_method_", clustering_method, ".pdf", sep=""), width=40, height=15)
...
rotate_pdf(
  page_rotation = 90,
  input_filepath = paste(base_dir, "tRNA_KOs/Output/Plots/dendrograms/RNA_pol_III_dist_method_",  distance_method, "_clust_method_", clustering_method, ".pdf", sep=""),
  output_filepath = paste(base_dir, "tRNA_KOs/Output/Plots/dendrograms/RNA_pol_III_dist_method_",  distance_method, "_clust_method_", clustering_method, ".pdf", sep=""),
  overwrite = TRUE
)
```
...
weighted_scores_df <- left_join(weighted_scores_df, master_temp, by = "Strain.Name")
cor(weighted_scores_df$weighted_score, weighted_scores_df$Family_size)
```
...
for (i in 1:nrow(go_terms_proteotoxicity)) {
  ...
  if (nrow(trna_temp) > 1) {
    create_heatmap(protein_mat = trna_temp,
                   ...
                   heatmap_type = "z-score",
                   z_score_type = "standard",
                   location_to_save = paste(base_dir, "tRNA_KOs/Output/Plots/Heatmaps/proteotoxicity/", sep=""),
                   save_as = "png",
