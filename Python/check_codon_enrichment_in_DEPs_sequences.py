...
# NEW VERSION, WITH THE 3 TYPES OF PROTEINS INCORPORATED INSTEAD OF 2 (this is, up-, down-regulated and non-DE, not just DE and non-DE).
# On 04.09.2025


"""
I now start from the da dataframe produced in R, where each row is a certain protein in a certain strain (the number of rows = number of strains*number of proteins detected).
There are columns with the information of whether each of these proteins is up-, down-regulated, or non DE in that strain. My goal is to, for each strain: 
    - Go through the up-regulated DE proteins, and in the FASTA file with all protein sequences, check how many times the codon in the KOd tRNA was present in the protein sequence
    - Do the same for the down-regulated DE proteins
    - And the same for the non DE proteins
    ...
    - Hence proving that the KO of a tRNA causes up- or down-regulation of, particularly, proteins containing more of the corresponding codon
"""
...
###############################################################
# 1.1. da dataframe
file_path = os.path.join(base_dir, "Data\\enrichment_analysis\\da_df_005_logFC_0.5.csv")
...
strains = list(da["Strain.Name"].unique())
for strain in strains:
    strain_dict_up = {}
    strain_dict_down = {}
    strain_dict_non_DE = {}
    strain_dict_just_counts = {"DE_up_regulated": [],
                               "DE_down_regulated": [],
                               "non_DE": []}
    # Come up with the codon
    anticodon =  strain[strain.find(".")+1:strain.rfind(".")]
    codon = anticodon_to_codon(anticodon)

    # Get the lists of DE proteins in this strain - one for up, one for down, if a protein is not in either, we know it's not DE
    up_regulated_proteins = da.loc[(da["Strain.Name"] == strain) & (da["diffexpressed_adjusted"] == "Up_regulated"), "protein"].tolist()
    down_regulated_proteins = da.loc[(da["Strain.Name"] == strain) & (da["diffexpressed_adjusted"] == "Down_regulated"), "protein"].tolist()
    ...
    for protein in list(ref_dict.keys()):
        ...
        if "/" in protein:
            ...
            if any(element in up_regulated_proteins for element in proteins):
                protein_new = next((element for element in up_regulated_proteins if element in proteins), None)
                strain_dict_up[protein_new] = ratio
                strain_dict_just_counts["DE_up_regulated"].append(ratio)
            elif any(element in down_regulated_proteins for element in proteins):
                protein_new = next((element for element in down_regulated_proteins if element in proteins), None)
                strain_dict_down[protein_new] = ratio
                strain_dict_just_counts["DE_down_regulated"].append(ratio)
            else:
                strain_dict_non_DE[protein] = ratio
                strain_dict_just_counts["non_DE"].append(ratio)

        else:
            if protein in up_regulated_proteins:
                strain_dict_up[protein] = ratio
                strain_dict_just_counts["DE_up_regulated"].append(ratio)
            elif protein in down_regulated_proteins:
                strain_dict_down[protein] = ratio
                strain_dict_just_counts["DE_down_regulated"].append(ratio)
            else:
                strain_dict_non_DE[protein] = ratio
                strain_dict_just_counts["non_DE"].append(ratio)
    ...
    final_count_dict_with_protein_names[strain] = {"DE_up_regulated": strain_dict_up,
                                                   "DE_down_regulated": strain_dict_down,
